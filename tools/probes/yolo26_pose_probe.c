// yolo26_pose_probe.c - 探针：yolo26n_pose.rknn（非 end2end）输出布局
// 预期 3 x [1, 56, H, W]，56 = 4(box直接距离) + 1(cls logit) + 51(17kpt x 3)
// 打印：输出属性 + 每通道 min/max/mean + 命中网格的 box/kpt 原始值，便于确定通道布局
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
int main(int argc, char** argv) {
    ...
    for (int oi = 0; oi < io.n_output; oi++) {
        ...
        for (int c = 0; c < C; c++) {
            ...
            printf("  ch%02d: min=%9.3f max=%9.3f mean=%8.3f\n", c, mn, mx, GL ? sum/GL : 0);
        }
    }

    // 打印 top 网格（按 cls logit 通道 ch4 的 sigmoid 排序），输出其 box + 前3个关键点的原始值
    for (int oi = 0; oi < io.n_output; oi++) {
        rknn_tensor_attr& a = out_attr[oi];
        int C = a.fmt == RKNN_TENSOR_NCHW ? a.dims[1] : a.dims[3];
        int H = a.fmt == RKNN_TENSOR_NCHW ? a.dims[2] : a.dims[1];
        ...
        printf("== OUT[%d] top cells (cls logit at ch4, sigmoid):\n", oi);
        struct Cell { int off; float s; };
        std::vector<Cell> cells;
        for (int g = 0; g < GL; g++) cells.push_back({g, sigmoid(p[4*GL + g])});
        std::sort(cells.begin(), cells.end(), [](const Cell& a, const Cell& b){ return a.s > b.s; });
        for (int k = 0; k < 8 && k < (int)cells.size(); k++) {
            int g = cells[k].off;
            int j = g % W, i = g / W;
            printf("  cell(%d,%d) stride=%d score=%.3f box=[%.1f %.1f %.1f %.1f]",
                   i, j, stride, cells[k].s,
                   p[0*GL+g], p[1*GL+g], p[2*GL+g], p[3*GL+g]);
            for (int kp = 0; kp < 3; kp++) {
                printf(" kp%d=(%.1f,%.1f,c%.2f)", kp, p[(5+kp*3)*GL+g], p[(5+kp*3+1)*GL+g], p[(5+kp*3+2)*GL+g]);
            }
            printf("\n");
        }
        // 用 r_pipe 采用的 decode 公式解码 top1 单元格，输出 17 个 kpt 在 640 图上的坐标
        if (!cells.empty()) {
            int g = cells[0].off;
            int j = g % W, i = g / W;
            printf("  [decode] cell(%d,%d) stride=%d: kpt_pix = (raw + cell + 0.5)*stride (Pose26), conf=sigmoid\n", i, j, stride);
            for (int kp = 0; kp < 17; kp++) {
                float rx = p[(5+kp*3)*GL+g], ry = p[(5+kp*3+1)*GL+g], rc = p[(5+kp*3+2)*GL+g];
                float kx = (rx + j + 0.5f) * stride;
                float ky = (ry + i + 0.5f) * stride;
                printf("    kp%02d raw=(%7.2f,%7.2f,c%6.2f) pix=(%6.1f,%6.1f) conf=%.3f\n",
                       kp, rx, ry, rc, kx, ky, sigmoid(rc));
            }
        }
    }
