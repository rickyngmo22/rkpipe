#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
导出 YOLO26-Seg「拆分布局」原始头 ONNX（one2one 头）。

输出签名（10 个输出）：
    box_i  : [1, 4,  H, W]   4 box 直接距离（网格单位，reg_max=1）
    cls_i  : [1, nc, H, W]   分类 logits（默认 clamp 到 [--clamp-lo, --clamp-hi]）
    mask_i : [1, 32, H, W]   mask 系数（不 clamp，独占 scale）
    proto  : [1, 32, 160, 160] 原型掩膜

为什么拆分（detect/obb 同根源，obb 已实测验证）：
  融合布局 [1, 4+nc+32, H, W] 的输出张量共享量化 scale，80 类 COCO 的 cls logits
  极端负值把 box/mask 一起撑爆 → 拆分后 box/cls/mask 各用各的 scale。
  mask 系数与 box 值域差异也大（box 可达数十网格单位，coef 通常 ±10 内），一并拆开。

运行环境：WSL onnx_env（ultralytics）
用法：
  python export_onnx_seg_split.py --weights seg/yolo26n-seg.pt --output onnx/yolo26n-seg_split10.onnx
"""
import argparse
import os
import types

import torch

from ultralytics.nn.modules.head import Segment26


def main():
    parser = argparse.ArgumentParser(description="YOLO26-Seg box/cls/mask 拆分导出")
    parser.add_argument("--weights", required=True, help="未 fuse 的 yolo26*-seg.pt checkpoint")
    parser.add_argument("--output", default="onnx/yolo26n-seg_split10.onnx")
    parser.add_argument("--imgsz", type=int, default=640)
    parser.add_argument("--opset", type=int, default=12)
    parser.add_argument("--clamp-lo", type=float, default=-12.0)
    parser.add_argument("--clamp-hi", type=float, default=8.0)
    parser.add_argument("--no-clamp", action="store_true", help="不加 clamp（对照组）")
    args = parser.parse_args()

    from ultralytics import YOLO

    os.makedirs(os.path.dirname(args.output) or ".", exist_ok=True)

    print(f"[load] {args.weights}")
    model = YOLO(args.weights).model
    model.eval()
    try:
        model.fuse()
        print("[ok] 模型已 fuse")
    except Exception as e:
        print(f"[warn] fuse 失败，继续导出: {e}")

    head = model.model[-1]
    assert isinstance(head, Segment26), f"不是 Segment26 头: {type(head).__name__}"
    assert hasattr(head, "one2one_cv2"), "模型无 one2one 分支"
    assert head.reg_max == 1, f"reg_max 应为 1（直接距离回归），实际 {head.reg_max}"
    print(f"[head] {type(head).__name__} | nl={head.nl} nc={head.nc} reg_max={head.reg_max} nm={head.nm}")

    no_clamp = args.no_clamp
    clamp_lo, clamp_hi = args.clamp_lo, args.clamp_hi

    def raw(self, x):
        outs = []
        for i in range(self.nl):
            box = self.one2one_cv2[i](x[i])     # [1, 4, H, W]
            cls = self.one2one_cv3[i](x[i])     # [1, nc, H, W]
            msk = self.one2one_cv4[i](x[i])     # [1, 32, H, W]
            if not no_clamp:
                cls = torch.clamp(cls, clamp_lo, clamp_hi)
            outs.append(box)
            outs.append(cls)
            outs.append(msk)
        proto = self.proto(x)
        if isinstance(proto, tuple):
            proto = proto[0]
        outs.append(proto)
        return tuple(outs)

    head.forward = types.MethodType(raw, head)

    dummy = torch.randn(1, 3, args.imgsz, args.imgsz)
    with torch.no_grad():
        out = model(dummy)
    shapes = [tuple(o.shape) for o in out]
    assert len(shapes) == 3 * head.nl + 1, f"输出数异常: {len(shapes)} != {3 * head.nl + 1}"
    for s in shapes:
        print(f"  -> {s}")

    output_names = []
    for i in range(head.nl):
        output_names += [f"box_{i}", f"cls_{i}", f"mask_{i}"]
    output_names.append("proto")

    print(f"[export] {args.output} (opset={args.opset}, imgsz={args.imgsz}, "
          f"clamp={'off' if no_clamp else f'[{clamp_lo}, {clamp_hi}]'})")
    torch.onnx.export(
        model,
        (dummy,),
        args.output,
        opset_version=args.opset,
        input_names=["images"],
        output_names=output_names,
        dynamic_axes=None,
        dynamo=False,
    )
    print("[ok] 导出完成")


if __name__ == "__main__":
    main()
