...
- **io**:输入源抽象(`InputSource`:文件/图片目录/RTSP/v4l2)、输出路由(`OutputRouter`:文件、RTSP 推流、Web 预览)、帧的零拷贝释放、逐帧结果 JSONL 汇(`result_sink`:环境变量 `RK_PIPE_RESULT_JSONL`,schema 见 docs/event_payload.md)。
...
- **detection**:SimpleObjectTracker(ID + 平滑 + 遮挡恢复)、叠绘、事件规则引擎(EventEngine:绊线/入侵/滞留/离岗/聚集/遗留物/跌倒/超速,支持命名多规则,纯逻辑可单测)。多人姿态场景开启 tracking 后,跟踪 ID 会为每个人锁定骨架/框颜色,避免人物进出画面时变色。
...
更多任务类型与多任务组合将在后续版本提供;契约头(`include/rkpipe/`、`include/core/`)的演进见仓库 Release 说明。
