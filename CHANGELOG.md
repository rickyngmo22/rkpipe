...
### 新增
- **rk_pipe_daemon 常驻守护（多路检测控制台）**：`build/rk_pipe_daemon --streams-file <streams.json> --api-port <port>` 读多路配置（每路 task/input/model/threads/pace/port/tracking），fork 监督 N 个 `console_detector --config` 子进程；内嵌控制台页（单文件无 CDN 依赖）卡片式管理各任务并 MJPEG 直连各路 `/stream.mjpg` 预览；REST API（`/api/health`、`/api/tasks`、`/api/summary`、`/api/events`+SSE、`/metrics` Prometheus、`/api/reload`）；`--watch` 配置热切换、`--restart` 异常自动重启；事件持久化与 LLM 复查（`configs/llm.yaml.sample`，缺省 mock 兜底、api_key 走环境变量不入配置）。板上实测：双路 detect（yolov8n + yolo26n @ 1080p replay）各路独立预览流，`kill -9` 子进程 10s 内自动复活；`console_detector` 新增 `--config` 形式调用兼容（daemon 监督使用）。样例配置见 `configs/streams/`
