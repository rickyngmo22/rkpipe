...
- **契约层**:契约头(`include/rkpipe/`、`include/core/`)的演进提案,请先开 issue 讨论再动代码(契约变更影响 ABI 兼容);
...
- 与既有代码风格保持一致(include 分组:系统 → 第三方 → 项目;仓库根有 .clang-format,提交前可对改动文件执行 `clang-format -i`);
- 公共头文件注释面向使用者,说明契约与线程语义,不写实现细节;
- 新后处理实现请同时补 CI 可跑的纯函数用例(tests/test_unit.cpp,并在 main 的注册表与 CMakeLists 的测试清单中各加一条);
- 源文件统一使用 `.cc` 扩展名。
