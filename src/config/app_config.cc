...
void AppConfig::printUsage(const char* program_name) {
    ...
    std::printf("  --track-max-missed <num> Max missed frames before dropping track (default: 20)\n");
...
AppConfig AppConfig::fromCommandLine(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if ((std::strcmp(argv[i], "-c") == 0 || std::strcmp(argv[i], "--config") == 0) && i + 1 < argc) {
            ...
            config.loadFromFile(config.config_path);
            break;
        }
    }
    ...
    int opt = 0;
    while ((opt = getopt_long(argc, argv, "m:i:v:o:t:d:j:pl:k:gc:C:N:LXI:M:H:R:D:S:P:QB:E:G:ZTAFU:", long_options, nullptr)) != -1) {
        switch (opt) {
            case 'm':
            ...
            case 'l':
                ...
                break;
            case 'k':
                config.obj_class_num = std::stoi(optarg);
                break;
            case 'g':
                config.gui = true;
                break;
            case 'c':
                ...
                break;
            case 'C':
                config.conf_threshold = std::stof(optarg);
                break;
            case 'N':
            ...
            case 'U':
                ...
                break;
            case 'I':
                config.track_iou_threshold = std::stof(optarg);
                break;
            case 'M':
                config.track_max_missed = std::stoi(optarg);
                break;
            case 'H':
                config.track_min_confirm_hits = std::stoi(optarg);
                break;
            case 'R':
                config.track_reid_iou_threshold = std::stof(optarg);
                break;
            case 'D':
                config.track_center_distance_threshold = std::stof(optarg);
                break;
            case 'S':
                config.track_box_smooth_alpha = std::stof(optarg);
                break;
            case 'P':
                config.track_render_max_missed = std::stoi(optarg);
                break;
            case 'B':
                config.track_high_conf_threshold = std::stof(optarg);
                break;
            case 'E':
                config.track_low_conf_threshold = std::stof(optarg);
                break;
            case 'G':
                config.track_stage2_iou_threshold = std::stof(optarg);
                break;
            case 'Z':
                config.track_size_ratio_threshold = std::stof(optarg);
                break;
            case 'T':
                config.track_max_tracks = std::stoi(optarg);
                break;
            case 'A':
