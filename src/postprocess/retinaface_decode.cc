...
std::vector<float> retinafacePriorBoxes(int model_size) {
    static const int kSteps[3] = {8, 16, 32};
    static const int kMinSizes[3][2] = {{16, 32}, {64, 128}, {256, 512}};
    static const int kGrids[3] = {40, 20, 10};  // 320/steps

    std::vector<float> priors;
    priors.reserve(4200 * 4);
    const float fs = static_cast<float>(model_size);
    for (int layer = 0; layer < 3; ++layer) {
        const int grid = kGrids[layer];
        for (int i = 0; i < grid; ++i) {          // 行
            for (int j = 0; j < grid; ++j) {      // 列
                for (int m = 0; m < 2; ++m) {     // 每位置 2 个 min_size
