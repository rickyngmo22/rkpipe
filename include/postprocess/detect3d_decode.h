...
// alpha = ry - theta（theta = atan2(Z, X)）→ ry = alpha + theta。
...
inline bool computeDetect3DCorners2D(const Detect3DItem& it, const float* p2, float out[8][2]) {
    ...
    const float alpha = std::atan2(it.sin_alpha, it.cos_alpha);
    const float theta = std::atan2(Z, X);
    const float ry = alpha + theta;
